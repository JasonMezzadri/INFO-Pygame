...
def draw_hud(surface: pygame.Surface, remaining: float, lives: int):
    ...
    timer_rect = timer_text.get_rect(centerx=SCREEN_W // 2, top=10)
...
# IMPLEMENTAZIONE BARRA TIMER
def draw_timer_bar(surface: pygame.Surface, remaining: float, duration: int):
    bar_x, bar_y, bar_w, bar_h = 50, 48, SCREEN_W - 100, 10
    # Sfondo
...
while running:
    ...
    if not game_over:
        ...
        # Gestione timer scaduto
        if is_expired(start_time, COUNTDOWN):
            game_over = True
            won = True if lives > 0 else False
    ...
    # Disegno elementi gioco
    paddle.draw(screen)
    ball.draw(screen)
    
    # Disegno HUD sopra tutto
