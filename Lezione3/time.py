...
def time_remaining(start: float, duration: int) -> float:
    """Restituisce i secondi rimanenti rispetto a duration (minimo 0)."""
...
def bar_fill_width(remaining: float, duration: int, bar_width: int) -> int:
    ...
    # Clamp per sicurezza tra 0 e bar_width
    return max(0, min(bar_width, fill))
...
def draw_timer_bar(surface: pygame.Surface,
    ...
    """Disegna la barra del timer (sfondo + riempimento)."""
    # 1. Disegna lo sfondo
    bg_rect = pygame.Rect(BAR_X, BAR_Y, BAR_W, BAR_H)
    pygame.draw.rect(surface, BAR_BG_COLOR, bg_rect, border_radius=6)
    
    # 2. Disegna il riempimento
...
def draw_timer_text(surface: pygame.Surface,
    ...
    """Disegna il numero dei secondi rimanenti o il messaggio di fine."""
    y_pos = BAR_Y + BAR_H + 10
    
    if expired:
        text_surf = font_large.render("Tempo scaduto!", True, TEXT_COLOR)
    else:
        # Usiamo ceil per mostrare ad esempio "10" finché non passa il primo millisecondo
        seconds_text = str(math.ceil(remaining))
        text_surf = font_medium.render(seconds_text, True, TEXT_COLOR)
    
    text_rect = text_surf.get_rect(centerx=SCREEN_W // 2, top=y_pos)
    surface.blit(text_surf, text_rect)


def draw_ball(surface: pygame.Surface,
              x: int, y: int, expired: bool):
    """Disegna la pallina con il colore appropriato allo stato."""
